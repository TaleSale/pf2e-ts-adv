...
class fallofplaguestoneJournalSheet extends JournalSheet {
  getData(options) {
  ...
  static get defaultOptions() {
    ...
    options.classes.push("fallofplaguestone");
    return options;
  }
  ...
  async activateListeners(html) {
    ...
    if (navigationElement) {
      navigationElement.style.background = "url('modules/pf2e-ts-adv/fall-of-plaguestone/asset/back.webp') no-repeat center center";
    ...
    separator.style.backgroundColor = '#013220';  // бордовый цвет

    html[0].appendChild(separator);

    // Загружаем слова для выделения из JSON-файла
    try {
      const response = await fetch('modules/pf2e-ts-adv/fall-of-plaguestone/keywords.json');
...
Hooks.once('init', () => {
  DocumentSheetConfig.registerSheet(JournalEntry, 'fallofplaguestone', fallofplaguestoneJournalSheet, {
    label: "Тема \"Падение Чумного Камня\"",
    makeDefault: false
  });
});
