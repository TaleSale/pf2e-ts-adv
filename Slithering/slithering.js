...
class SlitheringJournalSheet extends JournalSheet {
  getData(options) {
  ...
  static get defaultOptions() {
    ...
    options.classes.push("slithering");
    return options;
  }
  ...
  async activateListeners(html) {
    ...
    if (navigationElement) {
      navigationElement.style.background = "url('modules/pf2e-ts-adv/Slithering/back.webp') no-repeat center center";
...
Hooks.once('init', () => {
  DocumentSheetConfig.registerSheet(JournalEntry, 'slithering', SlitheringJournalSheet, {
    label: "Тема \"Ползучая Смерть\"",
    makeDefault: false
  });
});
