...
class SoGJournalSheet extends JournalSheet {
  getData(options) {
  ...
  async activateListeners(html) {
    ...
    if (navigationElement) {
      navigationElement.style.background = "url('modules/pf2e-ts-adv/journals/SoG/back.webp') no-repeat center center";
    ...
    try {
      const response = await fetch('modules/pf2e-ts-adv/journals/SoG/keywords.json');
...
Hooks.once('init', () => {
  DocumentSheetConfig.registerSheet(JournalEntry, 'SoG', SoGJournalSheet, {
    label: "Тема \"Сезон Призраков\"",
    makeDefault: false
  });
});
