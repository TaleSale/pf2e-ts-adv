...
class PFS3JournalSheet extends JournalSheet {
  getData(options) {
  ...
  async activateListeners(html) {
    ...
    if (navigationElement) {
      navigationElement.style.background = "url('modules/pf2e-ts-adv/journals/PFS3/back.webp') no-repeat center center";
    ...
    try {
      const response = await fetch('modules/pf2e-ts-adv/journals/PFS3/keywords.json');
...
Hooks.once('init', () => {
  DocumentSheetConfig.registerSheet(JournalEntry, 'PFS3', PFS3JournalSheet, {
    label: "Тема \"PFS3\"",
    makeDefault: false
  });
});
