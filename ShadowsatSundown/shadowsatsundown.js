...
class ShadowsAtSundownJournalSheet extends JournalSheet {
  getData(options) {
  ...
  static get defaultOptions() {
    ...
    options.classes.push("shadowsatsundown");
    return options;
  }
  ...
  async activateListeners(html) {
    ...
    if (navigationElement) {
      navigationElement.style.background = "url('modules/pf2e-ts-adv/ShadowsatSundown/asset/back.webp') no-repeat center center";
    ...
    separator.style.backgroundColor = '#800000';  // бордовый цвет

    html[0].appendChild(separator);

    // Загружаем слова для выделения из JSON-файла
    try {
      const response = await fetch('modules/pf2e-ts-adv/ShadowsatSundown/keywords.json');
...
Hooks.once('init', () => {
  DocumentSheetConfig.registerSheet(JournalEntry, 'shadowsatsundown', ShadowsAtSundownJournalSheet, {
    label: "Тема \"Тени на Закате\"",
    makeDefault: false
  });
});
