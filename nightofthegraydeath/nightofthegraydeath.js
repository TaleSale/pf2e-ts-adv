...
class NightofthegraydeathJournalSheet extends JournalSheet {
  getData(options) {
  ...
  static get defaultOptions() {
    ...
    options.classes.push("nightofthegraydeath");
    return options;
  }
  ...
  async activateListeners(html) {
    ...
    if (navigationElement) {
      navigationElement.style.background = "url('modules/pf2e-ts-adv/nightofthegraydeath/asset/back3.webp') no-repeat center center";
      navigationElement.style.backgroundSize = "cover";
      navigationElement.style.width = '300px';  // установим ширину навигационной части
    }

    if (contentElement) {
      contentElement.style.backgroundColor = "#url('modules/pf2e-ts-adv/nightofthegraydeath/asset/back.webp') no-repeat center center";  // светло-серый фон
    }
    ...
    separator.style.backgroundColor = 'rgb(26,17,31)';  // цвет

    html[0].appendChild(separator);

    // Загружаем слова для выделения из JSON-файла
    try {
      const response = await fetch('modules/pf2e-ts-adv/nightofthegraydeath/keywords.json');
...
Hooks.once('init', () => {
  DocumentSheetConfig.registerSheet(JournalEntry, 'nightofthegraydeath', NightofthegraydeathJournalSheet, {
    label: "Тема \"Ночь Серой Смерти\"",
    makeDefault: false
  });
});
